...
class Meanshift(Simple):
    ...
    def run(self, ips, imgs, para = None):
        ...
        for i in range(n):
            ...
            layer['body'].append({'type':'rectangle', 'body':(x+w/2,y+h/2,w,h)})
            mark['body'][i] = layer

        ips.mark = GeometryMark(mark)
        IPy.show_table(pd.DataFrame(locs, columns= ['X','Y','W','H']), ips.title+'-region')
class Camshift(Simple):
    ...
    def run(self, ips, imgs, para = None):
        ...
        ips.mark = GeometryMark(mark)
        IPy.show_table(pd.DataFrame(locs, columns=  ['P1_X','P1_y','P2_X','P2_y','P3_X','P3_y','P4_X','P4_y',]), ips.title+'-region')
